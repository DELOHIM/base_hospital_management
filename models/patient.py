...
class Patients(models.Model):
    ...
    status = fields.Selection([('married', 'Marié'),
                               ('unmarried', 'Célibataire')],
                              string="Etat civil", required=True)
    gender = fields.Selection([('female', 'F'),
                               ('male', 'M'),
                               ('others', 'Autres')],
    ...
    blood_grp = fields.Many2one('hospital.blood', string="Groupe sanguin")
    ...
    group = fields.Selection([('hindu', 'Hindu'), ('muslim', 'Muslim'),
                              ('christian',
                               'Christian')], string="Réligion", help="can specify your religion")
    ...
    education_level = fields.Selection([('post', 'Post Graduation'),
                                        ...
                                        ('pre', 'Pre Graduation')],
                                       string="Niveau d'études", help="Education status of patient")

    house_level = fields.Selection([('good', 'Meilleure'),
                                    ('bad', 'Moyenne'),
                                    ('poor', 'Basse')],
                                   string="condition de vie", help="Specify your house's condition")
...
class Family(models.Model):
    ...
    deceased = fields.Selection([('yes', 'Oui'), ('no', 'Non')], string="Décédé")
    family_id = fields.Many2one('res.partner', string="ID de la famille")


class Evaluation(models.Model):
    _name = 'hospital.evaluation'
    _description = 'Évaluation'
    patient_id = fields.Many2one('res.partner', "ID de la famille")
    name = fields.Char(string="Nom")
    indication = fields.Char(string="Indication")
    doctor = fields.Char(string="Médecin")
    appointment = fields.Char(string="Rendez-vous")
    eval_date = fields.Date(string="Date")
    body_mass = fields.Char(string="IMC")
    eval_type = fields.Char(string="Type d'évaluation")


class Perinatal(models.Model):
    _name = 'hospital.perinatal'
    _description = 'Périnatal'
    code = fields.Char(string="Code")
    grandiva = fields.Char(string="Grandiva")
    admission_date = fields.Date("Date d'admission")
    dismiss = fields.Char(string="Sortie de l'hôpital")
    weeks = fields.Char(string="Semaines gestationnelles")
    abortion = fields.Integer(string="Avortement")
    patient_id = fields.Many2one('res.partner', string="ID")


class LabTest(models.Model):
    _name = 'hospital.lab'
    _description = 'Test de laboratoire'
    lab_test = fields.Char(string="Test de laboratoire")
    lab_type = fields.Char(string="Type de laboratoire")
    date_req = fields.Date(string="Date demandée")
    date_analysis = fields.Date(string="Date d'analyse")
    state = fields.Date(string="État")
    patient_id = fields.Many2one('res.partner', string="ID")
...
class Medicine(models.Model):
    ...
    medicine_id = fields.Many2one('hospital.medicine', string="Médicament")
    indication = fields.Char(string="Indication", help="un signe, un symptôme ou une condition médicale qui conduit à la "
                                                       "recommandation d'un traitement, d'un test ou d'une procédure")
    prescription = fields.Char(string="Référence de prescription")
    dose = fields.Char(string="Dose")
    dose_unit = fields.Char(string="Unité de dose")
    form = fields.Char(string="Forme")
    frequency = fields.Char(string="Fréquence")
    duration = fields.Char(string="Durée du traitement")
    period = fields.Char(string="Période de traitement")
    patient_id = fields.Many2one('res.partner', string="ID")
